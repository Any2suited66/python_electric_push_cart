...
#define ELECTRIC_BRAKE_MAX    10       // (0, 500) Maximum electric brake to be applied when input torque request is 0 (pedal fully released).
#define ELECTRIC_BRAKE_THRES  20       // (0, 500) Threshold below at which the electric brake starts engaging.
...
#define ADC_MARGIN                300     // ADC input margin applied on the raw ADC min and max to make sure the MIN and MAX values are reached even in the presence of noise
#define ADC_PROTECT_TIMEOUT       100     // ADC Protection: number of wrong / missing input commands before safety state is taken
#define ADC_PROTECT_THRESH        1500     // ADC Protection threshold below/above the MIN/MAX ADC values
...
#define DEFAULT_RATE                160   // 20.0f [-] lower value == slower rate [0, 32767] = [0.0, 2047.9375]. Do NOT make rate negative (>32767) - Reduced for gentler braking
#define DEFAULT_FILTER              19660  // Default for FILTER 0.1f [-] lower value == softer filter [0, 65535] = [0.0 - 1.0].
#define DEFAULT_SPEED_COEFFICIENT   14336 // Default for SPEED_COEFFICIENT 1.0f [-] higher value == stronger. [0, 65535] = [-2.0 - 2.0]. In this case 16384 = 1.0 * 2^14
...
*/
  // #define DUAL_INPUTS                     // ADC*(Primary) + PWM(Auxiliary). Uncomment this to use Dual-inputs
    ...
    #define PRI_INPUT1            3, 1000, 1500, 2000, 100  // TYPE, MIN, MID, MAX, DEADBAND. See INPUT FORMAT section
    #define PRI_INPUT2            3, 1000, 1500, 2000, 100  // TYPE, MIN, MID, MAX, DEADBAND. See INPUT FORMAT section
  #endif
